...
class BookingCancellatorService
{
    ...
    public function execute(BookingCancellatorRequest $request)
    {
        ...
        $statusCancelled = $this->doctrine
            ->getRepository(Status::class) //ERROR: El repositori ha d'estar injectat
        ...
        if (!$booking->isNotRefundable() && $allowRefund) {
            ...
            // we get all costs
            $costs = $booking->getProviderCosts(); //ERROR: Això s'ha de recalcular aquí?
            ...
            $booking->recalculatePayout(); // ERROR: Aquest codi no hauria d'estar a la entitat => en un servei.
        }
        ...
        $this->doctrine->getManager()->flush(); //ERROR: Al repositori.

        if($request->isSendERPToProvider()){
            $this->erpSenderService->sendErp($booking, BookingERPStatus::ACTION_CANCEL);
        }

        if(!$request->isSendMailtoGuest()) { //ERROR: Aquest if està al revés!!!
